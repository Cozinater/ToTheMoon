...
function AssetsPage() {
  ...
  if (isError || !draft)
    return (
      <ErrorState
        message="Couldn't load your balances."
        onRetry={() => refetch()}
      />
    );

  function entriesOf(t: Target): Entry[] {
    return t.group === "assets"
      ? draft!.assets[t.key]
      : draft!.liabilities[t.key];
  }
  ...
  return (
    <>
      <PageHeader eyebrow="ASSETS (SGD)" title="What you own" />
      <div className="grid gap-4">
        {ASSET_SECTIONS.map((s) => (
          <SectionCard
            ...
            onAdd={() =>
              setForm({ group: "assets", key: s.key, title: s.title })
            }
            onEdit={(e) =>
              setForm({ group: "assets", key: s.key, title: s.title, entry: e })
            }
            onDelete={(e) =>
              setDeleting({
                group: "assets",
      ...
      <PageHeader eyebrow="LIABILITIES (SGD)" title="What you owe" />
      <div className="grid gap-4">
        {LIABILITY_SECTIONS.map((s) => (
          <SectionCard
            ...
            onAdd={() =>
              setForm({ group: "liabilities", key: s.key, title: s.title })
            }
            onEdit={(e) =>
              setForm({
                group: "liabilities",
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes it from the current draft only — closed months are
              untouched.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => {
                if (deleting?.entry) {
                  saveList(
                    deleting,
                    entriesOf(deleting).filter(
                      (e) => e.id !== deleting.entry!.id,
                    ),
                  );
                }
                setDeleting(null);
